...
class Json2TextTable:
    ...
    def convert_list(self, list_input):
        ...
        if column_headers is not None:
            ...
            #converted_output += '\n|2 ' + '|3 '.join(column_headers) + '|4 '
            converted_output += '\n| ' + '| '.join(column_headers) + '| '
            converted_output += '</thead>'
            converted_output += '<tbody>'
            for list_entry in list_input:
                #converted_output += '|5 <tr><td>'
                converted_output += '|1 '
                #converted_output += '|6 </td><td>'.join([self.convert_json_node(list_entry[column_header]) for column_header in
                converted_output += '|2 '.join([self.convert_json_node(list_entry[column_header]) for column_header in
                                                     column_headers])
                converted_output += '|3 '
            #converted_output += '</tbody>'
